from __future__ import annotations

import torch


def build_card_embedding(
    model: torch.nn.Module,
    full_tensor: torch.Tensor,
    symbol_tensor: torch.Tensor,
    name_tensor: torch.Tensor,
) -> torch.Tensor:
    """Erzeugt ein gemeinsames, L2-normalisiertes Karten-Embedding (Full + Symbol + Name)."""
    if full_tensor.ndim == 3:
        full_tensor = full_tensor.unsqueeze(0)
    if symbol_tensor.ndim == 3:
        symbol_tensor = symbol_tensor.unsqueeze(0)
    if name_tensor.ndim == 3:
        name_tensor = name_tensor.unsqueeze(0)
    with torch.no_grad():
        embedding = model.encode_normalized(full_tensor, symbol_tensor, name_tensor)
    return embedding.squeeze(0)
